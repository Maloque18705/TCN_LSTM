...
class DataProcess:
    ...
    def create_sample(self, data: np.ndarray, input_steps: int, output_steps: int,
        ...
        if limit_samples is not None:
            ...
            train_samples = int(limit_samples * len(train_data) / total_len)
            val_samples   = int(limit_samples * len(val_data) / total_len)
            test_samples  = limit_samples - train_samples - val_samples
        else:
