...
Usage:
    python run_tcn.py --epochs 100 --batch-size 64 --outdir ./outputs
...
from Model.tcn import TCN_Model
...
def parse_args():
    ...
    p.add_argument("--outdir", type=str, default=f"./outputs/tcn/{config.OUTPUT_STEPS}p", help="Base output directory")
...
def main():
    ...
    run_dir = base_out / f"run_TCN_{stamp}"
    run_dir.mkdir(parents=True, exist_ok=True)

    print(f"Starting TCN training. Artifacts will be saved to: {run_dir}")
    ...
    if hist and isinstance(hist, dict):
        try:
            if 'loss' in hist:
                ...
                plt.xlabel('Epochs')
                plt.ylabel('Loss (MSE)')
                plt.title(f"Training and Validation MSE Loss - TCN - Missing {config.OUTPUT_STEPS}% Data")
                plt.legend()
                plt.grid(True)
                plt.tight_layout()
    ...
    print("TCN run finished. Artifacts saved to:", run_dir)
